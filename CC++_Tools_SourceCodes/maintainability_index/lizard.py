...
def create_command_line_parser(prog=None):
    ...
    parser.add_argument("-M", "--MI",
                        help='''Threshold for maintainability index
                        warning. The default value is %d.
                        Functions with MI less than it will generate warning
...
class FunctionInfo(object):  # pylint: disable=R0902
    ...
    def compute_maintainability(self,unique,non_unique):
        if unique != 0:
            volume = non_unique * math.log(unique, 2)
            self.maintainability_index = int((171 - 5.2 * math.log(volume,2) - 0.23 * self.cyclomatic_complexity - 16.2 * math.log(self.nloc,2))*100 / 171)
    ...
    def clang_format_warning(self):
        return (
            ...
            " %(cyclomatic_complexity)d CCN, %(maintainability_index)d Maintainability and %(parameter_count)d" +
...
class FileInformation(object):  # pylint: disable=R0903
    ...
    maintainability_index = property(
        lambda self: self.functions_average("maintainability_index"))
...
class CodeInfoContext(object):
    ...
    def compute_maintainability(self, unique, non_unique):
        self.current_function.compute_maintainability(unique,non_unique)
...
class CLikeReader(CodeReader, CCppCommentsMixin):
    ...
    def _state_imp(self, token):
        ...
        if token == '{':
            ...
            if self.br_count == 0:
                self._state = self._state_global
                self._compute_maintainability()
                self.context.end_of_function()

    def _compute_maintainability(self):
        pure = []
        pure_unique = set()
        unique = 0
        non_unique = 0
        for t in self.function_tokens:
            if not(t == '}' or t == ')' or t == ',' or t == ';'):
                pure.append(t)
                pure_unique.add(t)
        pure.append(self.context.current_function.name)
        pure_unique.add(self.context.current_function.name)
        pure.append("{")
        pure_unique.add("{")
        pure.append("(")
        pure_unique.add("(")        
        for t in self.context.current_function.parameters:
            pure.append(t)
            pure_unique.add(t)
        unique = len(pure_unique)
        non_unique = len(pure)
        self.context.compute_maintainability(unique,non_unique)
        ...
        if file_info:
            for fun in file_info.function_list:
                if fun.cyclomatic_complexity > option.CCN or \
                   fun.maintainability_index < option.MI or \
...
class OutputScheme(object):
    ...
    def __init__(self, ext):
        ...
        self.items.append({'caption': " Maintainability  ", 'value': 'maintainability_index'})
...
def print_warnings(option, scheme, warnings):
    ...
    if not option.warnings_only:
        print(("\n" +
               "======================================\n" +
               "!!!! Warnings (CCN > %d) or (MI < %d)!!!!") % (option.CCN,option.MI))
...
def print_total(warning_count, saved_result, option):
    ...
    total_info = (
        ...
        float(sum([f.maintainability_index for f in all_fun])) / cnt,
        float(sum([f.token_count for f in all_fun])) / cnt,
        cnt,
        warning_count,
        float(warning_count) / cnt,
        float(sum([f.nloc for f in all_fun
                   if f.cyclomatic_complexity > option.CCN or f.maintainability_index < option.MI]))
        / nloc_in_functions
    )

    if not option.warnings_only:
        print("=" * 90)
        print("Total nloc  Avg.nloc  Avg CCN  Avg Maintainability Avg token  Fun Cnt  Warning" +
              " cnt   Fun Rt   nloc Rt  ")
        print("-" * 90)
...
def print_and_save_modules(all_modules, extensions, scheme):
    ...
    print("==============================================================")
    print("NLOC    Avg.NLOC AvgCCN AvgMaintainability Avg.ttoken  function_cnt    file")
    print("--------------------------------------------------------------")
    for module_info in all_functions:
        print((
            ...
            "{module.maintainability_index:7.1f}" +
